...
function dateTimeLabel(v){return window.BROUX?.date(v,true)||'—';}
...
async function load(){if(!inquiryId)return notice('No inquiry was selected.','error');const [iRes,aRes,jRes]=await Promise.all([db.from('leads').select('*').eq('id',inquiryId).single(),db.from('appointments').select('*').eq('lead_id',inquiryId).is('deleted_at',null).order('appointment_at',{ascending:false}),db.from('jobs').select('*').eq('lead_id',inquiryId).is('deleted_at',null).order('created_at',{ascending:false}).limit(1)]);if(iRes.error)return notice(iRes.error.message,'error');inquiry=iRes.data;window.BROUX?.record('Inquiry',inquiry.id,['#'+(inquiry.lead_number||'—'),inquiry.homeowner_name].filter(Boolean).join(' · '));appointments=aRes.data||[];linkedJob=(jRes.data||[])[0]||null;const cid=exactContactId();if(cid)await loadContact(cid);fill();renderAppointments();const requested=new URLSearchParams(location.search).get('appointment');if(requested){const match=appointments.find(a=>a.id===requested);if(match)openAppointment(match);}$('jobBtn').style.display='inline-flex';$('jobBtn').classList.add('primary');$('jobBtn').textContent=linkedJob?'Open Job':'Create Job';$('jobBtn').onclick=async()=>{
 if(linkedJob){location.href=`jobs.html?job=${encodeURIComponent(linkedJob.id)}`;return;}
 if(inquiryHasUnsavedChanges)return notice('Save your inquiry changes before creating a job.','error');
 if(!window.BROJobHandoff?.open)return notice('Job entry is still loading. Try Create Job again in a moment.','error');
 await window.BROJobHandoff.open({...inquiry,lead_id:inquiry.id,customer_name:contactRecord?.name||inquiry.homeowner_name,contact_id:inquiry.contact_id||contactRecord?.id||null});
};}
async function start(){await window.BROUX?.ready;installContextMenu();installContactEditor();db=supabase.createClient(cfg.SUPABASE_URL,cfg.SUPABASE_ANON_KEY);const {data,error}=await db.auth.getSession();if(error)return notice(error.message,'error');if(!data.session){location.href='index.html';return;}await load();}
...
let inquiryHasUnsavedChanges=false;
document.addEventListener('input',e=>{if(e.target.closest('section.card2')&&!e.target.closest('#appointmentList'))inquiryHasUnsavedChanges=true;});
document.addEventListener('change',e=>{if(e.target.closest('section.card2')&&!e.target.closest('#appointmentList'))inquiryHasUnsavedChanges=true;});
