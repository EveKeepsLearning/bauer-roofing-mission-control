...
(function(){
  ...
  async function refreshSuggestion(input,box){
    if(!input||!box)return;box.classList.add('loading');box.innerHTML='Checking next inquiry number…';
    try{const n=await nextAvailableNumber();box.dataset.suggested=n;box.innerHTML=`Suggested next inquiry #: <b>${esc(n)}</b> <button type="button" class="btn small">Use ${esc(n)}</button>`;box.querySelector('button').onclick=async()=>{const fresh=await nextAvailableNumber();input.value=fresh;input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));box.dataset.suggested=fresh;box.innerHTML=`Using inquiry #<b>${esc(fresh)}</b> <button type="button" class="btn small">Recheck</button>`;box.querySelector('button').onclick=()=>refreshSuggestion(input,box);};}
    catch(e){box.textContent=`Could not suggest a number: ${e.message||e}`;}finally{box.classList.remove('loading');}
  }
