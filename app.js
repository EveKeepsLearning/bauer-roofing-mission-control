...
let db=null,user=null,state={tasks:[],jobs:[],communications:[],incoming:[],phone:[],sops:[],suggestions:[]};
const $=id=>document.getElementById(id);
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const todayISO=()=>new Date().toLocaleDateString('en-CA',{timeZone:'America/New_York'});
const localNow=()=>new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',weekday:'short',month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'}).format(new Date());
function msg(text,type=''){const e=$('systemMsg');e.textContent=text;e.className='notice '+type;e.classList.remove('hidden');setTimeout(()=>e.classList.add('hidden'),5000)}
function authMsg(text,type=''){const e=$('loginMsg');e.textContent=text;e.className='notice '+type;e.classList.remove('hidden')}
function setView(name){document.querySelectorAll('[id^="view-"]').forEach(x=>x.classList.toggle('hidden',x.id!==`view-${name}`));document.querySelectorAll('#nav button').forEach(x=>x.classList.toggle('active',x.dataset.view===name));if(name==='playbook')renderSops();if(name==='suggestions')renderSuggestions()}
...
function renderSops(){const sel=$('sopSelect');sel.innerHTML=state.sops.map(s=>`<option value="${esc(s.id)}">${esc(s.title)}</option>`).join('');const show=()=>{const s=state.sops.find(x=>x.id===sel.value);$('sopBody').innerHTML=s?`<h3>${esc(s.title)}</h3><p><b>Purpose:</b> ${esc(s.purpose||'')}</p><p><b>When:</b> ${esc(s.when_to_use||'')}</p><p><b>Prerequisites:</b> ${esc(s.prerequisites||'')}</p><p><b>Steps:</b><br>${esc(s.instructions||'').replace(/\n/g,'<br>')}</p><p><b>Verify:</b> ${esc(s.how_to_verify||'')}</p>`:empty('No SOP selected.')};sel.onchange=show;show()}
...
async function saveJob(){const customer=$('jobCustomer').value.trim();if(!customer){msg('Customer name is required.','error');return}const row={customer_name:customer,lead_number:$('jobLead').value.trim(),property_address:$('jobAddress').value.trim(),salesperson:$('jobSalesperson').value.trim(),stage:$('jobStage').value,target_start_date:$('jobTarget').value||null,confirmed_start_date:$('jobConfirmed').value||null,production_notes:$('jobNotes').value.trim()};const {data,error}=await db.from('jobs').insert(row).select().single();if(error){msg(error.message,'error');return}state.jobs.push(data);$('jobDialog').close();renderDashboard();msg('Job created.','success')}
async function generateSuggestions(){const {error}=await db.rpc('generate_suggestions');if(error){msg(error.message,'error');return}const r=await db.from('suggestions').select('*').order('created_at',{ascending:false}).limit(100);if(r.error){msg(r.error.message,'error');return}state.suggestions=r.data||[];renderSuggestions();msg('Analysis complete.','success')}
async function init(){if(!cfg.SUPABASE_URL||!cfg.SUPABASE_ANON_KEY||String(cfg.SUPABASE_URL).includes('YOUR-')){document.body.innerHTML='<div class="login"><div class="card"><h1>Setup needed</h1><p>Create <b>config.js</b> from <b>config.example.js</b> and enter your Supabase project URL and anon key.</p></div></div>';return}db=supabase.createClient(cfg.SUPABASE_URL,cfg.SUPABASE_ANON_KEY);const {data:{session}}=await db.auth.getSession();await handleSession(session);db.auth.onAuthStateChange((_e,s)=>handleSession(s))}
async function handleSession(session){user=session?.user||null;$('loginView').classList.toggle('hidden',!!user);$('appView').classList.toggle('hidden',!user);if(user){try{await db.rpc('bootstrap_bauer_data');await db.rpc('ensure_recurring_tasks');await loadAll()}catch(e){msg(e.message||String(e),'error')}}}
$('loginBtn').onclick=async()=>{const email=$('loginEmail').value.trim();if(!email)return authMsg('Enter your email.','error');const {error}=await db.auth.signInWithOtp({email,options:{emailRedirectTo:location.href.split('#')[0]}});if(error)authMsg(error.message,'error');else authMsg('Check your email for the sign-in link.','success')};$('logoutBtn').onclick=()=>db.auth.signOut();$('refreshBtn').onclick=async()=>{try{await db.rpc('ensure_recurring_tasks');await loadAll();msg('Refreshed.','success')}catch(e){msg(e.message,'error')}};document.querySelectorAll('#nav button').forEach(b=>b.onclick=()=>setView(b.dataset.view));document.body.addEventListener('click',e=>{const b=e.target.closest('[data-action]');if(!b)return;const card=b.closest('[data-task-id]');taskAction(card.dataset.taskId,b.dataset.action)});$('savePhoneBtn').onclick=savePhone;$('saveIncomingBtn').onclick=saveIncoming;$('newTaskBtn').onclick=()=>$('taskDialog').showModal();$('saveTaskBtn').onclick=saveTask;$('newJobBtn').onclick=()=>$('jobDialog').showModal();$('saveJobBtn').onclick=saveJob;$('generateSuggestionsBtn').onclick=generateSuggestions;init();
