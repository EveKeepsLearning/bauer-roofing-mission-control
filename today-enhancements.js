...
(function(){
  ...
  function installStyles(){
    ...
    style.textContent=`
      ...
      body.bro-today-compact #appView>header .nav button,body.bro-today-compact #appView>header .nav a{padding:6px 9px}
      ...
      .bro-future-task .task-due{font-weight:700;color:#5f6d7d}
  ...
  function todayIsVisible(){const view=document.getElementById('view-today');return !!view&&!view.classList.contains('hidden');}
  ...
  function organizeTaskDates(){
    ...
    list.querySelectorAll('.bro-task-date-heading').forEach(el=>el.remove());
    ...
    [...list.children].forEach(node=>{
      ...
      if(due&&due>today){node.classList.add('bro-future-task');if(!firstFuture)firstFuture=node;}
    });
    ...
    const card=list.closest('.today-work-card');
    if(card&&current.previousElementSibling?.classList?.contains('bro-task-date-heading'))current.previousElementSibling.remove();
    if(card)card.insertBefore(todayHeading,current);
    if(firstFuture){
  ...
  function removeWeekJobs(){document.getElementById('weekJobsCard')?.remove();}

  function ensureScopeDialog(){
    if(document.getElementById('taskDeleteScopeDialog'))return;
    const dialog=document.createElement('dialog');dialog.id='taskDeleteScopeDialog';
  ...
  function isRepeatingTask(task){return !!(task&&(task.recurring_rule_id||task.recurrence_series_id||(task.repeat_pattern&&task.repeat_pattern!=='None')));}
  function openDeleteScope(task){ensureScopeDialog();pendingTaskDelete=task;const name=document.getElementById('taskDeleteScopeName');if(name)name.textContent=task.task||'Repeating task';document.getElementById('taskDeleteScopeDialog').showModal();}

  async function deleteOccurrence(task){const {error}=await db.rpc('bauer_record_action',{p_table:'tasks',p_id:task.id,p_action:'delete',p_description:`Deleted one occurrence of ${task.task||'repeating task'}.`});if(error)throw error;}
  async function deleteSeries(task){
    if(task.recurring_rule_id){
      const stop=await db.from('recurring_rules').update({active:false,updated_at:new Date().toISOString()}).eq('id',task.recurring_rule_id);if(stop.error)throw stop.error;
      const rows=await db.from('tasks').select('id,status').eq('recurring_rule_id',task.recurring_rule_id).is('deleted_at',null);if(rows.error)throw rows.error;
      for(const row of rows.data||[]){if(['Completed','Cancelled','Skipped'].includes(String(row.status||'')))continue;const result=await db.rpc('bauer_record_action',{p_table:'tasks',p_id:row.id,p_action:'delete',p_description:`Deleted remaining occurrences of ${task.task||'repeating task'}.`});if(result.error)throw result.error;}
      return;
    }
    if(task.recurrence_series_id){
      const rows=await db.from('tasks').select('id,status').eq('recurrence_series_id',task.recurrence_series_id).is('deleted_at',null);if(rows.error)throw rows.error;
      for(const row of rows.data||[]){if(['Completed','Cancelled','Skipped'].includes(String(row.status||'')))continue;const result=await db.rpc('bauer_record_action',{p_table:'tasks',p_id:row.id,p_action:'delete',p_description:`Deleted remaining occurrences of ${task.task||'repeating task'}.`});if(result.error)throw result.error;}
      return;
    }
  ...
  async function refreshAfterChange(message,type='success'){try{if(typeof loadAll==='function')await loadAll();else if(typeof renderDashboard==='function')renderDashboard();}catch(error){console.warn('Today refresh failed',error);}moveCurrentTaskIntoList();removeWeekJobs();organizeTaskDates();syncTodayClass();if(typeof msg==='function'&&message)msg(message,type);}

  async function deleteCommunication(id){
    const communication=(state.communications||[]).find(item=>item.id===id);if(!communication)return;
    const job=(state.jobs||[]).find(item=>item.id===communication.job_id);const label=communication.purpose||communication.type||'communication reminder';const jobText=job?.customer_name?` for ${job.customer_name}`:'';
    if(!confirm(`Delete “${label}”${jobText}?\n\nThis removes only this reminder. It does not delete the job.`))return;
    const result=await db.from('communications').delete().eq('id',id);if(result.error)throw result.error;state.communications=(state.communications||[]).filter(item=>item.id!==id);await refreshAfterChange('Communication reminder deleted.');
  }
  async function completeCommunication(id){const result=await db.from('communications').update({status:'Completed',completed_at:new Date().toISOString(),completed_by:user?.email||'Eve',updated_at:new Date().toISOString()}).eq('id',id);if(result.error)throw result.error;await refreshAfterChange('Communication reminder completed.');}
  ...
  function overrideRenderers(){
    ...
    communicationDueCard=function(entry){
      ...
      const c=entry.item;const dueText=formatDueDateTime(c.due_date,c.due_time);const job=(state.jobs||[]).find(j=>j.id===c.job_id);
      const sourceNote=c.purpose==='Installation-day customer check-in'?'Auto-created when this job entered In Production.':'';
    ...
    renderNextUp=function(task){const target=document.getElementById('currentTask');if(!target)return;if(!task){target.innerHTML='';return;}target.innerHTML=`<div class="bro-current-inline"><div class="bro-current-label">CURRENT TASK</div>${taskCard(task)}</div>`;};
  }

  function installEvents(){
    document.addEventListener('click',async event=>{
      try{
        const communicationDelete=event.target.closest('[data-delete-communication]');if(communicationDelete){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();await deleteCommunication(communicationDelete.dataset.deleteCommunication);return;}
        const communicationComplete=event.target.closest('[data-complete-communication]');if(communicationComplete){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();await completeCommunication(communicationComplete.dataset.completeCommunication);return;}
        const taskDelete=event.target.closest('[data-record-action="delete"][data-record-table="tasks"]');
        if(taskDelete){const task=(state.tasks||[]).find(item=>item.id===taskDelete.dataset.recordId);if(task&&isRepeatingTask(task)){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();openDeleteScope(task);return;}}
        const scope=event.target.closest('[data-task-delete-scope]');
        if(scope){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();const choice=scope.dataset.taskDeleteScope;const dialog=document.getElementById('taskDeleteScopeDialog');if(choice==='cancel'){pendingTaskDelete=null;dialog?.close();return;}const task=pendingTaskDelete;if(!task){dialog?.close();return;}scope.disabled=true;try{if(choice==='all')await deleteSeries(task);else await deleteOccurrence(task);pendingTaskDelete=null;dialog?.close();await refreshAfterChange(choice==='all'?'Repeating task and remaining occurrences deleted.':'This task occurrence was deleted.');}finally{scope.disabled=false;}return;}
      }catch(error){console.error(error);if(typeof msg==='function')msg(error.message||String(error),'error');}
    },true);
  }

  async function install(){
    for(let i=0;i<80;i++){if(typeof communicationDueCard==='function'&&typeof taskCard==='function'&&typeof renderNextUp==='function'&&typeof renderDashboard==='function'&&typeof state!=='undefined'&&typeof db!=='undefined')break;await sleep(100);}
    if(typeof communicationDueCard!=='function'||typeof taskCard!=='function'||typeof renderDashboard!=='function')return;
    installStyles();ensureScopeDialog();overrideRenderers();installEvents();moveCurrentTaskIntoList();removeWeekJobs();syncTodayClass();
    const target=document.getElementById('view-today');
    if(target)new MutationObserver(()=>{moveCurrentTaskIntoList();removeWeekJobs();organizeTaskDates();syncTodayClass();}).observe(target,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
    try{renderDashboard();moveCurrentTaskIntoList();removeWeekJobs();organizeTaskDates();}catch(error){console.warn('Today enhancement initial render deferred',error);}
  }
