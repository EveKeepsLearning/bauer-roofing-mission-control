...
(function(){
  ...
  async function syncAppointment(database,a){if(!a?.id)return{ok:false,error:new Error('Save the appointment in BRO before adding it to Google Calendar.')};const lead=await loadLead(database,a.lead_id);try{const result=await post(payloadFor(a,lead,isCanceled(a)?'cancel':'upsert'));const patch={google_calendar_status:isCanceled(a)?'Canceled':'Synced',google_calendar_event_id:result.google_event_id||a.google_calendar_event_id||null,updated_at:new Date().toISOString()};const{error}=await database.from('appointments').update(patch).eq('id',a.id);if(error)throw error;return{ok:true,result,appointment:a,lead};}catch(error){try{await database.from('appointments').update({google_calendar_status:'Sync Error',updated_at:new Date().toISOString()}).eq('id',a.id);}catch(_){}return{ok:false,error,appointment:a,lead};}}
  async function deleteAppointment(database,a){if(!a?.id)return{ok:false,error:new Error('Appointment ID is missing.')};if(!a.google_calendar_event_id)return{ok:true,result:{action:'not_on_google'}};const lead=await loadLead(database,a.lead_id);try{const result=await post(payloadFor(a,lead,'cancel'));return{ok:true,result,appointment:a,lead};}catch(error){return{ok:false,error,appointment:a,lead};}}
  async function pullGoogle(database){const range=rollingWindow();const payload=await post({secret:getSecret(),action:'list',start_time:range.start,end_time:range.end,calendar_id:GOOGLE_CALENDAR_ID});const events=Array.isArray(payload.events)?payload.events:Array.isArray(payload.result?.events)?payload.result.events:[];if(!events.length)return{total:0};const now=new Date().toISOString();const rows=events.filter(e=>e&&e.google_event_id&&e.start_at).map(e=>({google_event_id:String(e.google_event_id),google_calendar_id:String(e.google_calendar_id||GOOGLE_CALENDAR_ID),calendar_name:String(e.calendar_name||'Bauer Roofing'),summary:e.summary||null,description:e.description||null,location_raw:e.location_raw||e.location||null,start_at:e.start_at,end_at:e.end_at||null,color_id:e.color_id||null,raw_payload:e.raw_payload||e,synced_at:now,updated_at:now}));const{error}=await database.from('calendar_events').upsert(rows,{onConflict:'google_calendar_id,google_event_id'});if(error)throw error;return{total:rows.length};}
