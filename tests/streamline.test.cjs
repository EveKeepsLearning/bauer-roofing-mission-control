...
assert.match(sales,/sales_stage\.is\.null,sales_stage\.not\.in\.\(Sold,Rejected\)/,'Sales Pipeline should exclude closed history from its default Supabase query');
assert.match(sales,/showClosed.+addEventListener\('change',loadData\)/,'Closed Pipeline history should load only when requested');
assert.match(sales,/relatedLeads=nextLeads\.filter/,'Closed history should not trigger appointment and job lookups');
const salesHtml=fs.readFileSync('sales.html','utf8');
assert.match(salesHtml,/sales-board\.js\?v=20260915-sales-pipeline2/,'Sales Pipeline should cache-bust the optimized board script');
