...
do $$ declare actor record; taskid uuid; otherid uuid; mid uuid; n int; begin
 ...
 perform set_config('request.jwt.claims',jsonb_build_object('sub',(select id from personal_test_ids where kind='one'),'role','authenticated')::text,true);
 insert into public.team_messages(recipient_id,body) values((select id from personal_test_ids where kind='two'),'Thread root') returning id into mid;
 perform set_config('request.jwt.claims',jsonb_build_object('sub',(select id from personal_test_ids where kind='two'),'role','authenticated')::text,true);
 insert into public.team_messages(recipient_id,body,reply_to) values((select id from personal_test_ids where kind='one'),'Thread reply',mid);
 if not exists(select 1 from jsonb_array_elements(public.bro_message_threads()->'rows') r where r->>'reply_to'=mid::text) then raise exception 'Reply missing from thread'; end if;
 insert into public.team_message_dismissals(message_id) values(mid);
 if not exists(select 1 from jsonb_array_elements(public.bro_message_threads()->'rows') r where r->>'id'=mid::text and (r->>'hidden')::boolean) then raise exception 'Personal delete failed'; end if;
 perform set_config('request.jwt.claims',jsonb_build_object('sub',(select id from personal_test_ids where kind='one'),'role','authenticated')::text,true);
 if exists(select 1 from public.team_message_dismissals where message_id=mid) then raise exception 'Other delete leaked'; end if;
 if not exists(select 1 from jsonb_array_elements(public.bro_message_threads()->'rows') r where r->>'id'=mid::text and not (r->>'hidden')::boolean) then raise exception 'Other person lost their copy'; end if;
 perform set_config('request.jwt.claims',jsonb_build_object('sub',(select id from personal_test_ids where kind='three'),'role','authenticated')::text,true);
 begin
  insert into public.team_messages(recipient_id,body,reply_to) values((select id from personal_test_ids where kind='one'),'Unauthorized thread reply',mid);
  raise exception 'Third user entered thread';
 exception when insufficient_privilege then null; end;
 begin
  insert into public.team_message_dismissals(message_id) values(mid);
  raise exception 'Third user deleted message';
 exception when insufficient_privilege then null; end;
end $$;
reset role;
select 'PASS: private tasks, subtasks, notes, editing and RPCs; shared job scheduling; participant-only messages, recipient receipts, complete threads, personal deletion, third-user thread rejection' as result;
rollback;
