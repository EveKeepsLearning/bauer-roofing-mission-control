...
(function(){
  ...
  function install(){if(typeof openBroContact==='function'){const original=openBroContact;openBroContact=async function(id){const result=await original(id);stripPreservedHistory();return result;};}
    if(typeof searchContacts==='function'){const originalSearch=searchContacts;searchContacts=async function(){const result=await originalSearch();stripArchiveResults();return result;};const searchBtn=document.getElementById('contactSearchBtn');if(searchBtn)searchBtn.onclick=searchContacts;}
    stripArchiveResults();stripPreservedHistory();
  }
